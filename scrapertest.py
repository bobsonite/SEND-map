...
CHUNK_FILE = 'urn_chunks/urn_chunk_1.csv'
OUTPUT_FILE = 'send_scrape_results_1.csv'
...
def scrape_ofsted_reports(playwright):
    urn_df = pd.read_csv(CHUNK_FILE)
    urn_list = [str(u) for u in URN_LIST]

    if os.path.exists(OUTPUT_FILE):
        existing = pd.read_csv(OUTPUT_FILE)
        done_urns = set(existing['URN'].astype(str))
    else:
    ...
    # ✅ Launch headless browser with stealth options
    ...
    context = browser.new_context(user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117 Safari/537.36")
